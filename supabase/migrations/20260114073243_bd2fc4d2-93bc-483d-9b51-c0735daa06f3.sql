...
-- FIX: Update purchase_package to award referral bonuses even if referral was already paid
-- Also ensure level 2 bonus is independent of level 1 referral record

CREATE OR REPLACE FUNCTION purchase_package(p_user_id UUID, p_package_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Add direct join bonus to purchaser's wallet
  UPDATE wallets 
  ...
  -- Create direct join bonus transaction for the purchaser
  INSERT INTO transactions (user_id, type, amount, description, reference_id)
  ...
  IF v_level1_referrer_id IS NOT NULL AND v_package.level1_bonus > 0 THEN
    ...
    -- Update team count for level 1 referrer
    UPDATE profiles 
  ...
  RETURN json_build_object(
    ...
    'direct_bonus', v_package.bonus_amount,
    'level1_referrer', v_level1_referrer_id,
    'level2_referrer', v_level2_referrer_id
  );
END;
$$;

-- FIX: Update make_direct_investment to use same logic
CREATE OR REPLACE FUNCTION make_direct_investment(p_user_id UUID, p_amount INTEGER)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Add direct bonus to investor's wallet
  UPDATE wallets 
  ...
  IF v_level1_referrer_id IS NOT NULL THEN
    ...
    -- Level 2 bonus: Referrer's referrer (5%)
    SELECT referred_by INTO v_level2_referrer_id FROM profiles WHERE id = v_level1_referrer_id;
